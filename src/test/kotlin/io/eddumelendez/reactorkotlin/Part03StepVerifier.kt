...
class Part03StepVerifier {
    ...
    fun expectFooBarError(flux: Flux<String>) {
        StepVerifier.create(flux).expectNext("foo", "bar").verifyError(RuntimeException::class.java)
    }
    ...
    fun expectSkylerJesseComplete(flux: Flux<User>) {
        StepVerifier
                .create(flux)
                .assertNext{ assert(it.username.equals("swhite")) }
                .assertNext{ assert(it.username.equals("jpinkman")) }
                .verifyComplete()
